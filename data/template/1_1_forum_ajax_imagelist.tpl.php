<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); if($imagelist) { ?><?php $i = 0;?><table cellspacing="2" cellpadding="2" class="imgl"><tr><?php if(is_array($imagelist)) foreach($imagelist as $image) { ?><?php $i++;?><td valign="bottom" id="image_td_<?php echo $image['aid'];?>" width="25%">
...
<?php if($i % 4 == 0 && isset($imagelist[$i])) { ?></tr><tr><?php } } if(($imgpad = $i % 4) > 0) { echo str_repeat('<td width="25%"></td>', 4 - $imgpad);; } ?>
</tr></table>
<?php if($_G['inajax']) { ?>
